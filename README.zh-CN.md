...
**Infinite Space** 把一个空白浏览器标签页变成思维的游乐场:在无缝的无限画布上任意放置**文字、图片、链接、Markdown、表格与图表**,把它们连成一张想法之网,跨**多个页面**、用**标签**分类整理,再交给**你选择的任意大模型**一键梳理、串成故事、或直接对话。

- **自带大模型(BYO-LLM)** —— 兼容任意 OpenAI 兼容接口(智谱 GLM、OpenAI、DeepSeek、Groq、OpenRouter、Ollama……)。你的 API Key 保存在服务端,**绝不暴露给浏览器**。
- **真正理解你画布内容的 AI** —— *AI 整理* 把相关节点聚类并生成带标签的整洁布局;*AI 总结* 把整块画布串成故事线、支持多风格重生成;*AI 对话* 与整块画布聊天;*AI 联想* 推荐相关节点;*AI 标注关系* 自动命名连线。
- **本地优先、可选全栈** —— 画布数据住在浏览器 localStorage;可选的 Express 后端补充服务端持久化、LLM 代理与链接元数据增强。同一份代码的纯静态(零后端)版也已发布在 GitHub Pages。
...
| 📄 **Markdown 节点** | GFM 实时渲染 + 代码块语法高亮,双击内联编辑源码 |
| 📊 **表格节点** | 内联电子表格编辑;配置好 LLM 后即可**一键生成图表** |
| 📈 **图表节点** | 由你的 LLM 根据表格数据自动绘制柱状 / 折线 / 饼图 |
| 🔗 **链接卡片** | 显示标题 + URL 的智能卡片,保存时**自动抓取页面标题与 favicon** |
| ⛓️ **节点连线** | 从边缘手柄拖出或连线工具连接任意节点,每条连线可编辑**关系说明标签** |
| 🏷️ **标签** | 给节点打标,标签丝带 + 筛选面板,**组合筛选**,并对筛选子集执行 AI 操作 |
...
| 💬 **AI 对话** | 与当前看板流式对话(SSE,可取消)——让它总结、扩写或起草新节点 |
| 💡 **AI 联想** | 为选中节点推荐语义相关节点,点击跳转或连线一触即达,按节点缓存 |
| 🏷️ **AI 标注关系** | 一键为所有未标注连线生成简短关系说明 |
| 🔍 **全局搜索** | `Cmd/Ctrl+F` 跨页面即输即搜 |
| 📄 **模板** | 从模板库一键生成初始画布 |
...
| **Markdown** | 工具栏 `M` | GFM 实时渲染 + 代码高亮;**双击**打开源码编辑。 |
| **表格** | 工具栏 `⨝` | 内联电子表格;配置好 LLM 后点「生成图表」即可让 AI 据此绘制图表。 |

### 节点连线

点击 **⛓ 连线** 工具,依次点击**起点**、**终点**节点即可生成连线;或 hover 任一节点,从四角的**连线手柄**拖出。连线**端点对端点**、带方向箭头。点击一条连线打开**关系卡片**,可输入简短说明(如 原因 / 属于 / 引用于),或一键 **AI 标注关系** 把所有未标注连线全部命名。连线很重要:**AI 总结会读取连线关系**,让故事线忠实反映画布的真实结构。

### 标签

选中节点后点 **`+` 标签** 即可打标。工具栏的 **标签面板** 列出所有标签及数量——点击标签**组合筛选**(隐藏非匹配节点),再对筛选子集执行 **AI 整理 / AI 总结**,或清除筛选。

### AI 功能

所有 AI 功能都使用 **⚙️ 设置** 中配置的大模型,且**在服务端执行**(Key 永不进入浏览器)。

- **✨ AI 整理** —— 一键。LLM 聚类相关节点,应用内自动将每个簇排版成带**自动分组标签**的网格、给成员自动打标,并适配视口。
...
- **💬 AI 对话** —— 打开侧栏,围绕当前看板**流式**对话,可让它总结、扩写或起草新节点(`Enter` 发送、`Shift+Enter` 换行、可点停止取消)。
- **💡 AI 联想** —— 选中一个节点后,推荐语义相关节点(可跳转或连线),**按节点缓存**,切换不丢。
- **🏷️ AI 标注关系** —— 一键为所有未标注连线提议简短关系说明,可在卡片中逐条修改后再应用。
...
├── .github/workflows/pages.yml  # 构建静态 demo 并发布到 GitHub Pages
