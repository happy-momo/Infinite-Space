// 浏览器端 LLM 封装：在 GitHub Pages 静态版（无后端）下直接调用任意 OpenAI 兼容的
// /chat/completions 接口。包含 AI 分组整理、看板/子集总结、AI 聊天（流式）、AI 联想、
// AI 关系标注、图表分析、连接测试。逻辑与 server/llm.ts 一致，返回结构也完全兼容
// 现有 App 的解析。key 来自用户本机 localStorage，只存在于浏览器、绝不写入仓库。
// Browser-side LLM wrapper — talks directly to any OpenAI-compatible /chat/completions
// for the static (no-backend) GitHub Pages build. Shaped identically to server/llm.ts.
...
async function fetchWithTimeout(
  ...
  // 外部 signal（如聊天停止按钮）触发时同样取消本次请求。
  const onOuterAbort = () => controller.abort();
  signal?.addEventListener('abort', onOuterAbort, { once: true });
  try {
    return await fetch(url, { ...init, signal: controller.signal });
  } catch (e: any) {
    // 外部 signal（用户点"停止"）主动取消 → 以 AbortError 冒泡，调用方据此判定为"已停止"而非报错。
    if (signal?.aborted) {
    ...
    // CORS 拦截在浏览器里通常表现为 TypeError，给出更友好的提示。
    if (e instanceof TypeError) {
      throw new LlmError('浏览器拦下了跨域请求。请换用允许浏览器跨域的服务商(如 Groq / OpenRouter / DeepSeek / 硅基流动)，或检查 Base URL');
    }
...
// 直接以 SSE + stream:true 调用 LLM，把增量文本逐个交给 onDelta。
// 传入外部 signal 可取消（对应"停止"按钮）。
...
// 与生成画布互补：对"现有节点/连线"产出增量操作（update/add/delete/link/unlink），
