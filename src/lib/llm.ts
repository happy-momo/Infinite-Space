// 浏览器端 LLM 封装:在 GitHub Pages 静态版(无后端)下直接调用任意
// OpenAI 兼容的 /chat/completions 接口,提供 AI 分组整理(organizeNodes)
// 与看板总结/故事线(summarizeBoard)。key 来自本机 localStorage,只存在用户浏览器。
// Browser-side LLM wrapper — talks directly to any OpenAI-compatible endpoint
// for the static (no-backend) GitHub Pages build. The key lives in localStorage.
...
async function fetchWithTimeout(url: string, init: RequestInit, timeoutMs: number): Promise<Response> {
  ...
  try {
    ...
    // CORS 拦截在浏览器里表现为 TypeError,给出更友好的提示。
    if (e instanceof TypeError) {
      throw new LlmError('浏览器拦下了跨域请求。请换用允许浏览器跨域的服务商(如 Groq / OpenRouter / DeepSeek),或检查 Base URL');
    }
