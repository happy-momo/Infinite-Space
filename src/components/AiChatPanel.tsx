// AI 对话侧边栏：与看板对话（浏览器端流式输出），可让 AI 总结/扩写/生成新节点，
// 也可把一段文字描述一键解构成节点 + 连线生成到画布。
// 消息状态由父组件（App）按画布分桶管理；本组件为受控组件，仅负责渲染与流式交互。
// AI chat sidebar — talk to your board with streaming. Owned by parent; rendering + streaming only.
